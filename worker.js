...
export default {
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname==="/api/feed")return json((await env.DADOS_KV?.get("feed",{type:"json"}))||{items:[],updatedAt:null});
...
async function activities(prefs,env){
  const base={lat:38.9586,lon:-77.3570}; // Reston city-level center, not a precise user address
  const miles=Math.max(10,Math.min(140,(Number(prefs.distance)||90)));
...
nwr(around:${radius},${base.lat},${base.lon})["name"]["leisure"~"water_park|sports_centre|escape_game|amusement_arcade|marina"];
nwr(around:${radius},${base.lat},${base.lon})["name"]["sport"~"climbing|karting|skiing|archery|canoe|kayak"];
);out center tags 80;`;
  const r=await fetch("https://overpass-api.de/api/interpreter",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded","user-agent":"DadOS/1.0"},body:"data="+encodeURIComponent(q)});
  if(!r.ok)throw new Error("Real-place search unavailable");
  const d=await r.json();
  let items=(d.elements||[]).map(e=>{
    ...
    return {name:t.name,kind:t.tourism||t.leisure||t.sport||"attraction",website:norm(t.website||t["contact:website"]||""),lat,lon};
  }).filter(Boolean);
  const seen=new Set();items=items.filter(x=>{const k=x.name.toLowerCase();if(seen.has(k))return false;seen.add(k);return true}).slice(0,45);
  if(!items.length)return {heading:"Nearby ideas",items:[]};
...
Prefer memorable, unusual, scenic, adventurous, technical, or experiential options over generic everyday places.
...
Candidates: ${JSON.stringify(items.map(x=>({name:x.name,kind:x.kind})))}`,env);
  ...
  return {heading:ranked.heading||"Nearby ideas",items:(ranked.items||[]).map(x=>{
    const real=map.get(x.name);if(!real)return null;
    return {name:real.name,kind:real.kind,why:x.why||"",website:real.website,mapsUrl:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(real.name+" near Reston Virginia")}`};
  }).filter(Boolean)};
}
